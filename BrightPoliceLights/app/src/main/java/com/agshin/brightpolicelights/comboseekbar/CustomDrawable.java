package com.agshin.brightpolicelights.comboseekbar;

import java.util.List;

import com.agshin.brightpolicelights.comboseekbar.ComboSeekBar.Dot;
...
public class CustomDrawable extends Drawable {
	...
	public final void draw(Canvas canvas) {
        ...
        float d = (float)(mDotRadius / 6.);

		for (Dot dot : mDots) {
			drawText(canvas, dot, dot.mX, height);
//            float md = (dot.id % 3 == 0? mDotRadius : mDotRadius / 2.f);
            float md = (dot.text.contains(".") || dot.text.length() == 1? mDotRadius : mDotRadius / 2.f);
//            if(dot.id % 4 == 0) md = mDotRadius * 1.1f;
...
//			canvas.drawCircle(dot.mX, height, mDotRadius, circleLinePaint);
            canvas.drawRect(dot.mX - d, height - md, dot.mX + d, height + md, circleLinePaint);
		}
	}
